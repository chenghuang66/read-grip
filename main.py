...
def display_result(pra_results, pra_pref='Train_epoch'):
	...
	overall_log = '|{}|[{}] All_All: {}'.format(
		datetime.now(),pra_pref, ' '.join(['{:.3f}'.format(x) for x in list(overall_loss_time) + [np.sum(overall_loss_time)]]))
...
# loader_train = data_loader(pra_traindata_path='./train_data.pkl',
# pra_batch_size=batch_size_train=64, pra_shuffle=True, pra_drop_last=True, train_val_test='train')
# 将已经预处理好的数据组成数据集
def data_loader(pra_path, pra_batch_size=128, pra_shuffle=False, pra_drop_last=False, train_val_test='train'):
	...
	# 定义了一个torch.utils.data.DataLoader的对象
...
def compute_RMSE(pra_pred, pra_GT, pra_mask, pra_error_order=2):
	...
	print('sum: ',torch.sum(x2y2))
	print('pra_mask: ', torch.sum(pra_mask))
...
def preprocess_data(pra_data, pra_rescale_xy):
	...
	# clone返回一个和源张量同shape、dtype和device的张量，与源张量不共享数据内存，但提供梯度的回溯
	# clone提供了非数据共享的梯度追溯功能，而detach又“舍弃”了梯度功能，
	# 因此clone和detach意味着着只做简单的数据复制，既不数据共享，也不对梯度共享，从此两个张量无关联
	data = ori_data.detach().clone()
	new_mask = (data[:, :2, 1:]!=0) * (data[:, :2, :-1]!=0)
	# takes velocity (∆ x, ∆ y) as input
...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
	...
	for iteration, (ori_data, A, _) in enumerate(pra_data_loader):
		...
		# 训练过程不是将前六帧数据作为输入,而是输入前两帧数据,然后循环递增输入,输出与之相反
		for now_history_frames in range(1, data.shape[-2]):
			...
			predicted = pra_model(pra_x=input_data, pra_A=A, pra_pred_length=output_loc_GT.shape[-2],
								  pra_teacher_forcing_ratio=0, pra_teacher_location=output_loc_GT) # (N, C, T, V)=(N, 2, 6, 120)
			...
			print('overall_sum_time.shape: ', overall_sum_time.shape)
			print('overall_num.shape: ', overall_num.shape)
			total_loss = torch.sum(overall_sum_time) / torch.max(torch.sum(overall_num), torch.ones(1, ))
			print('overall_sum_time.sum: ', torch.sum(overall_sum_time))
			print('overall_num.sum: ', torch.sum(overall_num))
...
def run_trainval(pra_model, pra_traindata_path, pra_testdata_path):
	...
	for now_epoch in range(total_epoch):
		# itertools.chain把可迭代对象变成迭代器, 这就有了迭代器的优点:惰性计算和节省内存
